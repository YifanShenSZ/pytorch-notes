...
Mask has to be some large negative value (say -1e8) rather than -inf
...
Args:
    ...
    attn_mask: 10 mask indicating locations that should not take part in softmax,
               1 means not, 0 means yes, shape (N, nheads, L_t, L_s)
...
"""
def mha_padded_autograd(query, key, value, nheads, attn_mask,
W_q, W_k, W_v, W_out,
...
dropout_p=0.0):
    ...
    # padding-specific step: add -1e8 mask for padding in softmax
    attn_mask = attn_mask.reshape((N * nheads, L_t, L_s))
    attn_weights = torch.baddbmm(attn_mask * -1e4, query, keyT)
    ...
    # padding-specific step: since it is -1e8 rather than -inf,
    #                        the hole-column padding is not nan,
    #                        so need a mask removal rather than simple nan_to_num(0.0)
    attn_weights = attn_weights * (1.0 - attn_mask)
